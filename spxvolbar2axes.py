...
def process_data():
    ...
    try:
        ...
        if not os.path.exists("spx_vol.csv"):
        ...
        ax2.set_yticklabels([f"{es:.1f}" for es in es_ticks])
        
        # Formatting
        ax.set_title(
            f"SPX Volume by Strike - Spot: {spotSPX:.2f}\n({datetime.now().strftime('%Y-%m-%d %H:%M')})", 
            pad=20, fontsize=14
        )
        ax.set_xlabel("Volume")
        ax.set_ylabel("SPX Strike Price")
        ax2.set_ylabel("/ES Equivalent", color='cyan')
