...
def process_data():
    ...
    try:
        ...
        spotSPY = float(df_raw.iloc[0, 2])  # Solo si lo usas más adelante
        ...
        df_filtered = df[(df["Strike"] >= min_strike) & (df["Strike"] <= max_strike)].copy()
         # Calcular valores de ES equivalentes
        ...
        # Dummy trace para activar el eje yaxis2
        fig.add_trace(go.Scatter(
            x=[None],  # evita mostrar puntos
            y=df_filtered["Strike"],
            yaxis="y2",  # <-- clave para activar el eje secundario
            mode="lines",
            line=dict(color='cyan'),
            showlegend=False
        ))

        tick_interval = 1  # muestra solo 1 de cada 2 strikes
        ...
        fig.update_layout(
            ...
            yaxis=dict(  # Eje izquierdo con valores SPX
                title="Strike",
                tickmode="array",
                tickvals=strikes[::tick_interval],
                ticktext=[f"{s:.0f}" for s in strikes[::tick_interval]],
                side="left",
                showgrid=False,
                color="white"
            ),
       
           # yaxis=dict(title="Strike", color="white", tickmode="linear", dtick=5),
            yaxis2=dict(
                overlaying='y', 
                side='right', 
                title='/ES',  
                tickmode="array",
                tickvals=strikes[::tick_interval],
                ticktext=[f"{v:.1f}" for v in df_filtered["ES_Equivalent"][::tick_interval]],
